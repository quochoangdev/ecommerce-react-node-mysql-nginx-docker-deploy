...
const Ipad = () => {
  ...
  const allVersion = [{ key: "Tất cả", value: "" }, { key: "IPad Mini", value: "mini" }, { key: "IPad Air", value: "air" }, { key: "IPad Air M2", value: "air m2" }, { key: "IPad Pro M2", value: "pro m2" }];
  ...
  useEffect(() => {
    fetchProducts("tablet", "ipad", null, null);
    setCurrentLimit(12);
  ...
  const handleClickItemCategory = (key, value) => {
    ...
    setVersion(value)
    fetchProducts("tablet", "ipad", value, sort);
  };

  const handleCategorySelect = async (e) => {
    if (e.target.value !== false) {
      setSort(e.target.value);
      fetchProducts("tablet", "ipad", version, e.target.value);
    }
  };
